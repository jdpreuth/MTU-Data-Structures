...
public class Labyrinth {
	...
	private Edge<Walkway>[] orderOutgoing(Vertex<RoomCoordinate> vertex) {
		/*
		 * TCJ: This makes O(1) comparisons from the AdjListGraph method
		 * outgoingEdges which is O(m) and stores them in an array
		 */
	...
	private void DFS(Vertex<RoomCoordinate> vertex, HashMap<Vertex<RoomCoordinate>, Boolean> visited,
		...
		/*
		 * TCJ: The DFS has to visit every edge and vertex in the graph as it
		 * travereses through to complete the depth-first search
		 */
	...
	public Iterable<Edge<Walkway>> dfsPath(RoomCoordinate start, RoomCoordinate end) {
		/*
		 * TCJ: The depth first search has to look at every vertex and edge in
		 * order to be able to construct a path from two given coordinates
		 */
	...
	public Iterable<Edge<Walkway>> bfsPath(RoomCoordinate start, RoomCoordinate end) {
		/*
		 * TCJ: The BFS algorithm has to look at every vertex and edge in the
		 * graph in order to find the information nessecary to construct the
		 * path
		 */
	...
	public Iterable<Edge<Walkway>> shortestPath(RoomCoordinate start, RoomCoordinate end) {
		/*
		 * TCJ: Djikstra's Algorithm to find the shortest path requires every
		 * vertex and edge to be put through a Heap PQ where the insert and
		 * remove methods cost O(log n) and all other data structures are
		 * HashMaps where the methods have an expected cost of O(1).
		 */
		...
		while (pq.size() > 0) {
			...
			for (Edge<Walkway> edge : orderOutgoing(v)) {
				if (edge != null && cloud.get(mGraph.opposite(v, edge)) == null) {
	...
	public static double totalPathDistance(Iterable<Edge<Walkway>> path) {
		/*
		 * TCJ: The distance stored in each edge has to be accessed and worse
		 * case the path contains every edge
		 */
		double distance = 0;
		for (Edge<Walkway> e : path) {
			distance += e.getElement().getDistance();
		}
		return distance;
	}
